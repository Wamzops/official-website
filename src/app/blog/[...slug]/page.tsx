...
export default async function BlogPost({
  ...
  if (!sectionSlug) {
    ...
    if (firstSection) {
      redirect(`/blog/${postSlug}/${firstSection.slug}`);
    }
  }
  ...
  // TOC from H2-H3 headings in current section
  const sectionHeadings = extractHeadings(currentSection.content);

  const isFirstSection = currentIdx === 0;

  return (
    <ContentLayout posts={sidebarPosts} category="blog" tocHeadings={sectionHeadings}>
      <Schema
        as="blogPosting"
        baseURL={baseURL}
        path={`${blog.path}/${slug.join("/")}`}
      ...
      <div>
        ...
        <div className="doc-pagination">
          {prevSection ? (
            <Link
              href={`/blog/${postSlug}/${prevSection.slug}`}
          ...
          {nextSection ? (
            <Link
              href={`/blog/${postSlug}/${nextSection.slug}`}
            ...
            <Link href="/blog" className="pagination-card next" prefetch>
              <span className="pagination-card-label">Done</span>
              <span className="pagination-card-title">Back to Blog</span>
            </Link>
          )}
        </div>
      </div>
